...
<body>
  ...
  <div class="page-container">
    <c:set var="x" value="${fn:split('综合检查表/定期检查表/整改实施','/') }"/>
    <div class="text-c">
    <c:if test="${flag == 1 }">
        ...
        <tbody>
          ...
          <c:forEach items="${list }" varStatus="index" var="be">
            ...
            <td>
              ...
              <c:if test="${be.type == 2}">
              <a style="text-decoration:none" onClick="show_dialog('自动化设置_${be.id }', '${ly}/company/plan-auto?modelId=${be.id }')" href="javascript:;">自动化设置</a><br/>
              </c:if>
        ...
        <tbody>
          ...
          <c:forEach items="${list }" varStatus="index" var="be">
            ...
            <td>
              ...
              <c:if test="${flag == 1}">
              <a style="text-decoration:none" onClick="show_dialog('检查详情_4rf4f4f${be.id }', '${ly}/company/check-detail?flag=${flag }&id=${be.id }')" href="javascript:;">查看检查记录</a>
              </c:if>
              <c:if test="${flag == 4 || flag == 3}">
              <a style="text-decoration:none" onClick="show_dialog('检查详情_5gg5g5g${be.id }', '${ly}/village/check-document?checkId=${be.id }&flag=8')" href="javascript:;">查看检查记录</a>
              </c:if>
  ...
  $.post(getRootPath() + "/company/plan-save2", {
    ...
    if(result.status == '1') {
      layer.alert(result.map.message);
        ...
        show_dialog('show_dialog_' + id_, getRootPath()+'/village/plan-next?id=' + id_);
    }
  })
}
$(function() {
