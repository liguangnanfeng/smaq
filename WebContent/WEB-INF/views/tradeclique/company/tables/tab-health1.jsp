...
<body>
  <nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> <span>职业健康“三同时”</span> 
  ...
  <div class="page-container">
    <div class="text-c">
    <form action="${ly }/company/tables/tab-health/1" method="post">
      <div class="dis-ib">
        <span>资料名称：</span>
      ...
      <div class="dis-ib">
        ...
        <select name="isType" class="select">
          ...
          <option value="1" <c:if test="${1==isType}">selected</c:if>>建设项目职业卫生“三同时”审查登记表</option>
          <option value="2" <c:if test="${2==isType}">selected</c:if>>建设项目批准文件</option>
          <option value="3" <c:if test="${3==isType}">selected</c:if>>职业病危害预评价委托书与预评价报告</option>
          <option value="4" <c:if test="${4==isType}">selected</c:if>>建设项目职业病防护设施设计专篇</option>
          <option value="5" <c:if test="${5==isType}">selected</c:if>>职业病危害控制效果评价委托书与控制效果评价报告</option>
          <option value="6" <c:if test="${6==isType}">selected</c:if>>建设单位对职业病危害预评价报告、职业病防护设施设计专篇、职业病防护设施控制效果评价报告的评审意见</option>
          <option value="7" <c:if test="${7==isType}">selected</c:if>>安全监管部门审核、审查、验收批文</option>
          <option value="8" <c:if test="${8==isType}">selected</c:if>>全套竣工图纸、验收报告、竣工总结</option>
          <option value="9" <c:if test="${9==isType}">selected</c:if>>工程变更的图纸及有关材料</option>
        </select>
        </span>
      </div>
      ...
      <table class="table table-border table-bordered table-bg table-hover table-sort">
        ...
        <tbody>
          ...
          <tr class="text-c">
            ...
            <td>
                <c:if test="${1==h.isType}">建设项目职业卫生“三同时”审查登记表</c:if>
                <c:if test="${2==h.isType}">建设项目批准文件</c:if>
                <c:if test="${3==h.isType}">职业病危害预评价委托书与预评价报告</c:if>
                <c:if test="${4==h.isType}">建设项目职业病防护设施设计专篇</c:if>
                <c:if test="${5==h.isType}">职业病危害控制效果评价委托书与控制效果评价报告</c:if>
                <c:if test="${6==h.isType}">建设单位对职业病危害预评价报告、职业病防护设施设计专篇、职业病防护设施控制效果评价报告的评审意见</c:if>
                <c:if test="${7==h.isType}">安全监管部门审核、审查、验收批文</c:if>
                <c:if test="${8==h.isType}">全套竣工图纸、验收报告、竣工总结</c:if>
                <c:if test="${9==h.isType}">工程变更的图纸及有关材料</c:if>
            </td>
            <td>
