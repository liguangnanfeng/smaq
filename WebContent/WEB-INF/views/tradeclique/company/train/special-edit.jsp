...
<div class="page-container">
  <form class="form form-horizontal" id="form">
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <input type="text" id="a" name=""  value="${spe.name }" style="width:350px" class="input-text required"  >
      </div> 
    </div>
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        ...
        <option value="" >请选择</option>
          <c:forEach items="${lib }" varStatus="index" var="sp">
			<option value="${sp.name }" <c:if test="${spe.type eq sp.name}">selected</c:if>>${sp.name }</option>
		  </c:forEach>
        </select>
      </div> 
    </div>
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <input type="text" id="b" name="" value="${spe.certificateNumber}" style="width:350px" class="input-text required"  >
      </div> 
    </div>
    <div class="row cl">
      <label class="form-label col-xs-4 col-sm-2">取证时间：</label>
      <div class="formControls col-xs-8 col-sm-9">
        <input type="text" id="c" value="${spe.forensicTime }" onfocus="WdatePicker({dateFmt:'yyyy-MM'})" name="safeTime1" class="input-text Wdate mb-5 mt-5" style="width:350px;">
      </div>
    </div>
    <div class="row cl">
      <label class="form-label col-xs-4 col-sm-2">下次复审时间：</label>
      <div class="formControls col-xs-8 col-sm-9">
        <input type="text" id="e" value="${spe.reviewTime }" onfocus="WdatePicker({dateFmt:'yyyy-MM'})" name="safeTime3" class="input-text Wdate mb-5 mt-5" style="width:350px;">
      </div>
    </div>
    <div class="row cl">
      <label class="form-label col-xs-4 col-sm-2">备注：</label>
      <div class="formControls col-xs-8 col-sm-9">
        <textarea id="f" class="textarea txtarea_sq">${spe.remark }</textarea>
      </div> 
    </div>
    <div class="row cl">
  ...
  function article_save_submit() {
    ...
    $.post(getRootPath() + "/company/train/special-save",{
      	id:id,
      	flag: flag,
      	name:$("#a").val(),
      	type:$("#type").val(),
         ...
         show_tab("持证上岗", getRootPath() + '/company/train/special-list?spFlag=${spFlag}');
  ...
  $(function() {
		...
		var i = '${spe.flag}';
		$("#specialAddName").text('编辑' + title[parseInt(i - 1)]);
	})
  </script>
