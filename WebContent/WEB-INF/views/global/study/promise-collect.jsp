...
<body>
  <nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> <span>承诺公告汇总</span>
        ...
        <tbody>
          ...
          <c:forEach items="${list }" varStatus="index" var="be">
            <tr class="text-c">
            	<td>${index.index + 1 }</td>
            	<td>${be.name }</td>
            	<td><a href="${ly }/global/study/promise-list?uid=${be.uid }&type=1">${be.MonthNum }</a></td>
            	<td>${be.TotalNum }</td>
            	<td><a href="${ly }/global/study/promise-list?uid=${be.uid }&type=0">查看详情</a></td>
            </tr>
          </c:forEach>
          <!-- 循环结束 -->
        </tbody>
