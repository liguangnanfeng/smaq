    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
      <span class="l">
      	<c:if test="${session_user.userType >= 6 || session_user.userName eq '港口' || session_user.userType == 5}">
      	  <a class="btn btn-primary radius ml-10" onclick="show_dialog('添加行政处罚告知书', '${ly }/village/danger/opinion-add?flag=3&flag2=2')" href="javascript:;">
            <i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加行政处罚告知书
          </a>
	      <a class="btn btn-primary radius  mt-5" onclick="show_dialog('添加行政（当场）处罚决定书（单位）', '${ly }/village/danger/opinion-add?flag=4&flag2=2')" href="javascript:;">
	         <i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加行政（当场）处罚决定书（单位）
	      </a>
	      <a class="btn btn-primary radius ml-10 mt-5" onclick="show_dialog('添加行政（当场）处罚决定书（个人）', '${ly }/village/danger/opinion-add?flag=5&flag2=2')" href="javascript:;">
	         <i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加行政（当场）处罚决定书（个人）
          </a>
	      <a class="btn btn-primary radius ml-10 mt-5" onclick="show_dialog('添加行政处罚决定书（单位）', '${ly }/village/danger/opinion-add?flag=6&flag2=2')" href="javascript:;">
	        <i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加行政处罚决定书（单位）
	      </a>
	      <a class="btn btn-primary radius ml-10 mt-5" onclick="show_dialog('添加行政处罚决定书（个人）', '${ly }/village/danger/opinion-add?flag=7&flag2=2')" href="javascript:;">
...
/*详情*/
function detail(id){
  show_dialog("文书详情", getRootPath() + "/village/danger/opinion-detail?id=" + id);
}
