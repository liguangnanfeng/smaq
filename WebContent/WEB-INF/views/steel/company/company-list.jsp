...
<body>
  ...
  <span class="c-gray en">&gt;</span> <span>所属企业</span>
  ...
  <div class="page-container">
    <div class="text-c">
      <form action="/steel/company/company-list" method="post">
        <div class="dis-ib">
        <span>企业名称：</span>
        ...
        <tbody>
         ...
         <c:forEach items="${list }" varStatus="index" var="co">
          <tr class="text-c">
            ...
            <%-- <td ondblclick="window.open('/tradeclique/move/company?un=${co.name }');"> --%>
            <td>
                <c:choose>
...
function user_isKey(userId,isKey){
  ...
  layer.confirm(text,function(){
    var index = layer.load();
    $.post(getRootPath() + "/village/user-isKey",{
      userId : userId,
      isKey : isKey
    },function(result){
      layer.close(index);
...
function deltrade(userId) {
  layer.confirm("确认要删除该企业吗？",function(){
    var index = layer.load();
    $.post(getRootPath() + "/trade/trade-user-del",{
      userId : userId,
    },function(result){
      layer.close(index);
      $("#page").val(0);
