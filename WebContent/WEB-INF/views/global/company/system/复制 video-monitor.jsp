...
function info(wndId){
	...
	//alert(camjson[wndId]);
	if('${flag}' == 2)
		show_tab(camjson[wndId], "/village/to_monitor-list?before=2&flag=3&userId=${company.userId}&name="+camjson[wndId]);
}
...
function update_resource(){
	...
	if(camreso.length==0)
		return;
	//alert(JSON.stringify(camreso));
	//var obj = new Object();
	var puids = camreso[0].attributes.pu.puid;
	var ids = camreso[0].attributes.self.idx;
	var names = camreso[0].attributes.self.name;
	var userId = '${userId}';
	...
	$.ajax({
	    type : "POST",
	    url : getRootPath() + '/village/monitor-update?puids='+puids+'&ids='+ids+'&names='+names+'&userId='+userId,
	...
	if(flag==1){//配置页
	  ...
	  for(var i=0;i<cameras.length;i++){
	  	html += "<tr class='text-c'>"
	  						...
	  						+"<td>"
    							+"<a style='text-decoration:none' onClick=\"show_tab('编辑摄像头', '/village/monitor-add?userId="+'${userId}'+"&name="+cameras[i]+"')\" href='javascript:;' title='编辑'>查看/编辑</a>"
              		+"<a style='text-decoration:none' onClick=\"show_tab('"+cameras[i]+"', '/village/to_monitor-list?userId="+'${userId}'+"&before=1&flag=3&name="+cameras[i]+"')\" href='javascript:;' title='删除'>查看监控</a>"
            		+"</td>"
	  				+"</tr>";
	  }
	  //alert(html);
