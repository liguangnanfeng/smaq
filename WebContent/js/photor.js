...
(function($) {
  ...
  }, data = [], evt = getSupportedEvents(), handlers = [], params;
  ...
  var methods = {
    init : function(options) {
      ...
      return this.each(function() {
        var root = $(this), galleryId = this.id || data.length, p = {}, // Current instance of gallery
        content = {}, thumbs = [], imageTemplate = {
          url : '',
          thumb : '',
          caption : '',
          width : 0,
    ...
    update : function() {
      ...
      function updateInstance(galleryId) {
        ...
        p.layer.css('transition-duration', '0s').css(methods.setIndent(galleryId, -100 * p.current));
      }
    },
    ...
    go : function(galleryId, target, duration) {
      ...
      p.layer.css('transition-duration', duration + 'ms')
    ...
    prev : function(galleryId) {
      var p = data[galleryId], onScreen = p.params.slidesOnScreen;
      if (p.current > 0) {
        methods.go(galleryId, p.current - 1);
      } else {
    ...
    loadSlides : function(galleryId, target) {
      var p = data[galleryId], onScreen = p.params.slidesOnScreen, from = target - onScreen < 0 ? 0 : target - onScreen, to = target + (onScreen * 2)
          - 1 > p.last ? p.last : target + (onScreen * 2) - 1;
    ...
    loadSlide : function(galleryId, target) {
      var p = data[galleryId], slide = p.root.find('.' + p.params.slide + '.' + p.params.modifierPrefix + target), slideImg = slide.find('.'
          + p.params.slideImg), alt = p.slides[target].caption;
      ...
      if (alt) {
        slideImg.addClass(p.params._alt).attr('data-alt', alt);
      }
    },

    loadThumbs : function(galleryId) {
      var p = data[galleryId], count = p.count, images = p.slides, loaded = 0;
      p.galleryThumbs = [];
    ...
    getThumbsSize : function(galleryId) {
      var p = data[galleryId], thumb = p.thumb;
    ...
    setCurrentThumb : function(galleryId, target, noEffects) {
      var p = data[galleryId];
      if (p.params.showThumbs == 'thumbs') {
        var frame = p.thumbFrame, styles = {}, current = p.galleryThumbs && p.galleryThumbs[target], thumbsW = p.thumbs.outerWidth(), layerW = p.thumbsLayer
            .outerWidth(), duration = noEffects ? '0s' : (p.params.duration * 0.8 / 1000) + 's', indent, validatedIndent;
        ...
        if (p.galleryThumbsLoaded) {
          ...
          frame.css('transition-duration', duration).css(styles);
          p.thumbsLayer.css('transition-duration', duration).css(methods.setIndent(galleryId, validatedIndent, 'px'));
        }
      }
    ...
    position : function(galleryId, target) {
      var p = data[galleryId], slide = p.root.find('.' + p.params.slide + '.' + p.params.modifierPrefix + target), img = p.slides[target], viewportRatio = p.viewportWidth
          / p.viewportHeight, imgRatio = img.width / img.height;
      // Algorithm
      if (p.viewportWidth > img.width && p.viewportHeight > img.height) {
        p.slides[target].algorithm = 'center';
        slide.removeClass(p.params._auto).addClass(p.params._center);
      } else {
        p.slides[target].algorithm = 'auto';
        slide.removeClass(p.params._center).addClass(p.params._auto);
      }

      // Orientation
      if (imgRatio >= viewportRatio) {
        p.slides[target].orientation = 'landscape';
        slide.removeClass(p.params._portrait).addClass(p.params._landscape);
      } else {
        p.slides[target].orientation = 'portrait';
        slide.removeClass(p.params._landscape).addClass(p.params._portrait);
      }
    },
      ...
      for (var i = 0, len = data.length; i < len; i++) {
        ...
        if (params.showThumbs == 'thumbs' && data[i].url) {
          thumbsHTML += '<span data-rel="' + i + '" class="' + params.thumb + ' ' + params.modifierPrefix + i + ' ' + data[i].classes + '">'
              + '<img src="' + data[i].thumb + '" class="' + params.thumbImg + '" data-rel="' + i + '">' + '</span>';
        }
        // Slides template
        slidesHTML += '<div class="' + params.slide + ' ' + params.modifierPrefix + i + ' ' + (data[i].html ? params._html : params._loading)
            + '" data-id="' + i + '">';
        if (data[i].html) {
  ...
  function getSupportedTransform() {
    var out = false, el = document.createElement('p');
  ...
  function getPrefixed(property) {
    var style = document.createElement('p').style, prefixes = ['ms', 'O', 'Moz', 'Webkit'];
    if (style[property] == '') {
      return property;
    }
  ...
  function bindControl(galleryId) {
    var p = data[galleryId], thumbs = p.thumbs, touch = {}, inlineLinks = p.viewport[0].getElementsByTagName('a'), inlineImages = p.viewport[0]
     ...
     */
    function slidesMove() {
      var resultIndent, onScreen = p.params.slidesOnScreen;
    ...
    function slidesEnd() {
      ...
      if (Math.abs(touch.shiftX) > p.viewportWidth * 0.05) {
        var shiftSlides = touch.shiftX / p.viewportWidth * p.params.slidesOnScreen, target;
        if (touch.shiftX < 0) {
     ...
     */
    function thumbsMove() {
      var indent = touch.shiftX + touch.thumbsStartX, limit = -1 * (p.thumbsLayerWidth - p.thumbsWidth);
    ...
    function thumbsEnd() {
      if (p.thumbsDragging && touch.isSlide) {
        ...
        p.thumbsLayer.css('transition-duration', '.24s').css(methods.setIndent(galleryId, p.thumbsIndent, 'px'));
      }
    }
    ...
    function calcTailAnimation(currentIndent, direction) {
      var speed = Math.abs(10 * touch.shiftX / (touch.t2 - touch.t1)), tail, limit;
  ...
  function bindKeyboard(galleryId) {
    ...
    handlers.keydown = function(e) {
      var key = e.which || e.keyCode, node = e.target.nodeName.toLowerCase(), contenteditable = !!e.target.attributes.contenteditable;
  ...
  function bindTransitionEnd(galleryId) {
    var p = data[galleryId], transitionEnd = ['webkitTransitionEnd', 'MSTransitionEnd', 'oTransitionEnd', 'transitionend'];
  ...
  function toggleSlides(galleryId, target) {
    ...
    for (var i = 0, len = p.count; i < len; i++) {
      var elem = p.root.find('.' + p.params.slide + '.' + p.params.modifierPrefix + i), onScreen = p.params.slidesOnScreen;
