        	...
        	<div class="row cl">
		      ...
		      <div class="formControls col-xs-8 col-sm-9">
		        <select id="uid" name="uid"  style="width:350px" class="input-text required">
		          ...
		          <option value="${clique.userId }"<c:if test="${d.userId == clique.userId}"> selected</c:if>>${clique.name }</option>
		          <c:forEach items="${branches }" varStatus="index" var="br">
		          	<option value="${br.userId }"<c:if test="${d.userId == br.userId}"> selected</c:if>>${br.name }</option>
		          </c:forEach>
		        </select>
		      </div> 
		    </div>
...
function article_save_submit() {
  ...
  $.post(getRootPath() + "/tradeclique/detection-save",{
    id : id,
    userId: $("#uid").val(),
    type : $("#a").val(),
    reportTime:$("#b").val(),
    ...
    ba:$("#ba").val()
       ...
       show_tab("检测检验",  getRootPath() +'/tradeclique/detection-list');
  })
}
</script>
</html>