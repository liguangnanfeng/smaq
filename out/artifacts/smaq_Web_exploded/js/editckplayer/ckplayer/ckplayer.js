...
function ckplayerConfig() {
	return {
		...
		style: { //风格部分内容配置，这里主要配置loading和logo以及广告的部分内容
			...
			logo: {
				file : 'data:image/png;base64,',
				align: 'left',
				vAlign: 'top',
				offsetX: -100,
				offsetY: 10
		...
		*/
		addVEvent: function() {
			...
			var eventEnded = function(event) {
				$("#timeProgressId_l").css("width",$("#timeProgressId_l").parent("div").width());
				$("#timeboId_l").css("left",$("#timeProgressId_l").parent("div").width()-14);
			...
			if(this.showFace) {
				this.interFace();
				thisTemp.css(thisTemp.CB['playbackrate'], 'display', 'none');
			}
		},
		/*
			内部函数
		...
		*/
		...
		interFace: function() {
			...
			*/
			...
			//构建控制栏的内容
			html += '<div id="to_play_buttons" class="' + playID + '" data-title="' + thisTemp.language['play'] + '">' + this.newCanvas(playID, bWidth, bHeight) + '</div>'; //播放按钮
			...
			html += '<div class="' + fullID + '"  id="fullScreenId" data-title="' + thisTemp.language['full'] + '">' + this.newCanvas(fullID, bWidth, bHeight) + '</div>'; //全屏按钮
			html += '<div class="' + escFullID + '"   id="escFullId"  data-title="' + thisTemp.language['escFull'] + '">' + this.newCanvas(escFullID, bWidth, bHeight) + '</div>'; //退出全屏按钮
			html += '<div class="' + dlineID + '-ra"></div>'; //分隔线
			html += '<div class="' + definitionID + '" data-title="' + thisTemp.language['definition'] + '"></div>'; //清晰度容器
			html += '<div class="' + dlineID + '-rb"></div>'; //分隔线
			html += '<div class="' + playbackRateID + '" data-title="' + thisTemp.language['playbackRate'] + '"  id="playbackRate"></div>'; //倍速
			...
			//构建控制栏内容结束
			//构建进度条内容
			this.getByElement(timeProgressBgID).innerHTML = '<div class="' + loadProgressID + '"></div><div id="timeProgressId_l" class="' + timeProgressID + '"></div>';
			this.getByElement(timeBOBGID).innerHTML = '<div id="timeboId_l" class="' + timeBOID + '"><div class="' + timeBWID + '"></div></div>';
			//构建进度条内容结束
...
//			}
			...
			var resize = function() {
				...
				if(thisTemp.full){
					$("#captureId").parent().hide();
				}else{
					$("#captureId").parent().show();
				}
		...
		*/
		prompt: function() {
			...
			if(!this.showFace && (undefined ==prompt || prompt.length<=0)) {
				return;
			}
			var thisTemp = this;
