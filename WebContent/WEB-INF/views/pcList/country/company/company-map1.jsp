...
<style>
	.main_map .left_map .selects select{width: 33%}
	.switch-left{
    	height: 26px;
    	vertical-align: top;
	}
    ...
    <div class="main_map clearfix">
        <div class="left_map">
            ...
            <div class="selects clearfix">
                ...
                <select ng-model="townId" class="input-text" ng-change="townChange()">
                ...
                <select ng-model="villageId" class="input-text">
                    <option value="">--请选择村--</option><!--注意这个地方要和下面的设置的值要一致 -->  
...
app.controller('appCtrl', function($scope, $filter, $compile, $http) {
  ...
  $scope.townChange = function() {
      $http({
	      method: 'GET',
	      url: '${ly }/common/getVillageByTown?id=' + $scope.townId
	  }).then(function successCallback(response) {
	          // 请求成功执行代码
	          $scope.villageList = response.data.map.list;
	          var obj = JSON.stringify(response.data.map);
	      }, function errorCallback(response) {
	          // 请求失败执行代码
	  });
  }
