    ...
    <div class="text-c">
        <c:if test="${flag == 1 }">
            <div id="spTab" class="btn-group" style="text-align: center;margin-bottom: 20px;margin-top:20px;">
                <a class="btn default ${flag == 1&&type==1 ? 'btn-primary' : 'radius'}"
                   href="${ly }/tradeclique/model-list-cx?flag=1&type=1">综合检查表</a>
                <c:if test="${rjcbxs == 1 }">
                    <a class="btn default ${flag == 1&&type==9 ? 'btn-primary' : 'radius'}"
                       href="${ly }/tradeclique/model-list-cx1r?flag=1&type=9">日检查表</a>
                </c:if>
                <a class="btn default ${flag == 1&&type==2 ? 'btn-primary' : 'radius'}"
                   href="${ly }/tradeclique/model-list-cx?flag=1&type=2">定期检查表</a>
                <a class="btn default ${flag == 1&&type==3 ? 'btn-primary' : 'radius'}"
                   href="${ly }/tradeclique/check-list1?flag=1&type=3">整改复查</a>
            </div>
        </c:if>
    </div>
    <div class="cl pd-5 bg-1 bk-gray mt-20">
        ...
        <span class="l">
          <a class="btn btn-primary radius"
             onclick="show_dialog('添加${x[type-1] }','${ly }/company/model-add?type=${type }&flag=1')"
             onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont"
                                                                  style="font-size:15px;">&#xe600;</i> 添加${x[type-1] }</a>
        </span>
        </c:if>
        <c:if test="${flag == 3 }">
        <span class="l">
            ...
            <div class="modal-content radius">
                ...
                <div class="modal-body">
                    <div class="list_jian list_zi">
                        <a class="btn_jian btn_an1" data-dismiss="modal" aria-hidden="true" data-title="企业自查-添加综合检查表"
                ...
                <c:forEach items="${list }" varStatus="index" var="be">
                    <tr class="text-c">
                        ...
                        <td>
                            ...
                            <c:if test="${be.type != 9}">
                                <a style="text-decoration:none"
                                   onClick="show_dialog('编辑检查表_${be.id }', '${ly}/company/model-edit?modelId=${be.id }')"
                                   href="javascript:;">编辑检查表</a><br/>
                            </c:if>
                            <c:if test="${be.type == 2 || be.type == 9}"><!-- zhangcl 2018.10.27-->
                            <a style="text-decoration:none"
                               onClick="show_dialog('自动化设置_${be.id }', '${ly}/company/plan-auto?modelId=${be.id }&type=${type}')"
                               href="javascript:;">自动化设置</a><br/>
                            </c:if>
