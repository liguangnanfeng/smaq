...
var id_ = '${e.id}';
function save_() {
  $.post(getRootPath() + "/tradeclique/health-save", {
    id : id_,
    userId:$("#uid").val(),
    type : $("#type").val(),
    time : $("#time").val(),
    ...
    if(result.status == '1') {
      layer.alert(result.map.message);
        ...
        show_tab('职业健康体检', '/tradeclique/health-list')
    }
  })
}
</script>
...
<div class="page-container">
  <form class="form form-horizontal" id="form" action="${ly }/tradeclique/health-save" method="post">
  	<div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <select id="uid" name="uid"  style="width:350px" class="input-text required">
          ...
          <option value="${clique.userId }"<c:if test="${e.userId == clique.userId}">selected</c:if>>${clique.name }</option>
          <c:forEach items="${branches }" varStatus="index" var="br">
          	<option value="${br.userId }"<c:if test="${e.userId == br.userId}">selected</c:if>>${br.name }</option>
          </c:forEach>
        </select>
      </div> 
    </div>
