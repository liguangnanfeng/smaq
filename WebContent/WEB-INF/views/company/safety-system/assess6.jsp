    ...
    <script type="text/javascript">
        ...
        function update() {
            layer.confirm("确定进行风险点重置？", function (r) {
                $.post(getRootPath() + "/company/safety-system/risk-list-add2", {}, function (result) {
                    location.reload();
                })
            })
        }
...
<nav class="breadcrumb text-c">
    <a class="btn-page" data-href="${ly }/company/safety-system/assess5" data-title="重大风险判定"
       onclick="Hui_admin_tab(this)" href="javascript:;">重大风险判定</a>
    <a class="btn-page" data-href="${ly }/company/safety-system/assess-list" data-title="较大风险判定"
       onclick="Hui_admin_tab(this)" href="javascript:;">较大风险判定</a>
    ...
    <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px;margin-right: 10px;"
       href="${ly }/company/threeLeft?leftBasic=2" title="返回">返回</a>
</nav>
<div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
      <span class="l">
       ...
       <%-- <a class="btn btn-primary radius" href="javascript:void(0);" onclick="update()">风险因素辨识 </a>--%>
        <a class="btn btn-danger radius" href="javascript:void(0)" onClick="del_bath()"><i class="Hui-iconfont"
                                                                                           style="font-size:15px;">&#xe609;</i> 删除</a>
        ...
        <a class="btn btn-success-outline radius ml-10" href="javascript:void(0)" onClick="show2()"><i class="Hui-iconfont" style="font-size:15px;">&#xe695;</i> 企业安全风险矩阵与风险等级判别标准</a> -->
         <c:if test="${industry eq '化工企业（危化生产、使用）'}">
             <a class="btn btn-danger radius" href="javascript:void(0)"
                onClick="show_dialog('企业评分','${ly }/company/safety-system/grade-table')">企业评分</a>
         </c:if>
    ...
    <c:if test="${industry eq '冶金行业' || industry eq '有色行业' || industry eq '建材行业'|| industry eq '机械行业'|| industry eq '轻工行业'|| industry eq '纺织行业'|| industry eq '烟草行业'|| industry eq '商贸行业' }">
        <a class="btn btn-danger radius" href="javascript:void(0)"
           onClick="show_dialog('工贸企业','${ly }/company/safety-system/danger-table')">工贸评定</a>
    </c:if>
    <a class="btn btn-success-outline radius" href="javascript:void(0)"
       onclick="show_dialog('作业条件风险程度评价','/company/safety-system/assess2?id=')"><i class="Hui-iconfont"
                                                                                   style="font-size:15px;"></i> LEC评测</a>
    <a class="btn btn-success-outline radius" href="javascript:void(0)"
       onclick="show_dialog('风险矩阵法评价','/company/safety-system/assess3?id=')"><i class="Hui-iconfont"
                                                                                style="font-size:15px;"></i> LS评测</a>
        ...
        <table class="table table-border table-bordered table-bg table-hover table-sort tab-ass">
            ...
            <c:forEach items="${list }" var="be" varStatus="index">
                <tr class="text-c">
                    ...
                    <td>
                        <input class="btn btn-danger-outline size-S radius ml-5" type="button" value="红色"
                               onclick="zpLevel_save_(${be.id},this)"/>
                        <input class="btn btn-warning-outline size-S radius ml-5" type="button" value="橙色"
                               onclick="zpLevel_save_(${be.id},this)"/>
                        <input class="btn btn-warning-outline size-S radius btn-huang ml-5" type="button" value="黄色"
                               onclick="zpLevel_save_(${be.id},this)"/>
                        <input class="btn btn-secondary-outline size-S radius ml-5 mr-5" type="button" value="蓝色"
                               onclick="zpLevel_save_(${be.id},this)"/>
                        <a class="btn size-S radius" style="text-decoration:none" onClick="del_(${be.id})"
            ...
            <div class="modal-body">
                <div class="row cl">
                    ...
                    <div class="formControls col-xs-8 col-sm-9" style="width: 80%;">
                        ...
                        <a class="btn btn-primary radius" href="${ly }/company/system/department-list"><i
                                class="Hui-iconfont">&#xe600;</i>添加车间/岗位/部位</a>
                    </div>
                </div>
