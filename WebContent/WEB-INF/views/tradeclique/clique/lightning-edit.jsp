...
<div class="page-container">
  <form class="form form-horizontal" id="form">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <select id="uid" name="uid"  style="width:350px" class="input-text required">
          ...
          <option value="${clique.userId }"<c:if test="${be.userId == clique.userId}">selected</c:if>>${clique.name }</option>
          <c:forEach items="${branches }" varStatus="index" var="br">
          	<option value="${br.userId }"<c:if test="${be.userId == br.userId}">selected</c:if>>${br.name }</option>
          </c:forEach>
        </select>
      </div> 
    </div>
  ...
  function article_save_submit() {
    ...
    $.post(getRootPath() + "/tradeclique/clique/lightning-save", {
      id : '${be.id }',
      userId : $("#uid").val(),
      project : $("#project").val(),
      content : $("#content").val(),
      ...
      remark : $("#remark").val()
        ...
        show_tab("防雷防静电", getRootPath() + '/tradeclique/clique/lightning-list');
