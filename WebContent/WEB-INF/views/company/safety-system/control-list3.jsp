  <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
    ...
    href="javascript:endEditArea();" title="保存企业范围">
    保存企业范围
    </a>
    <a class="btn btn-success radius r" style="line-height: 1.6em; margin-top: 3px;margin-right: 10px;"
    href="javascript:startEditArea();" title="修改企业范围">
    修改企业范围
    </a>
    </nav>
    <img id="areaImg">
    ...
    <a href="${ly }/company/safety-system/jietu" data-title="截图"
    ...
    <script type="text/javascript"
    src="https://webapi.amap.com/maps?v=1.4.1&key=441462b46b736497a5f2a377aafb5b69&plugin=AMap.PolyEditor"></script>
    ...
    addImg()
    function addImg(){
    var str = localStorage.getItem('basicType')
    var str1 = localStorage.getItem('basicI')
    if(str==='1'){
    ...
    res = res
    if(res){
    localStorage.setItem('basicType', '2');
    $("#areaImg").attr("src",res);
    $("#areaImg").css({'width':'100%','height':'auto'})
    ...
    window.location.href = "/company/safety-system/area-range-save?arearange="+event.target.getPath().join(',');
    });
    polyEditor.open();
    }
    <%-- 保存范围方法 --%>
    ...
    //内部区域
    var path=[];
    polyEditor.Cn[0].map(function(item) {
    path.push(new AMap.LngLat(item.lng,item.lat,true));
    })
    var holes=[path];
    var outer = [
    ...
    window.location.href = "/company/safety-system/control-list3?id="+id+"&lnglat="+lnglat;
    }
