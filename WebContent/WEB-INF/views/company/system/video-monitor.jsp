	<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
		...
		var partimg = $(obj).closest("td").attr("data-img");
		window.Id = $(obj).closest("td").attr("data-id");
		if(window.Id==-1){
		alert("请先同步视频源！");
		return;
		}
		...
		$("td[data-id='"+Id+"']").attr("data-img", $("#pic4").attr("url"));
		$("#modal-img").modal("hide");
		$.post(getRootPath() + "/village/monitor-img-save", {
		id :Id,
		img : $("#pic4").attr("url")
		}, function(result){
      ...
      <script>
		//layer.msg('1')
			...
			<a class="btn btn-primary radius"  style="padding:4px 60px;float:right;text-decoration:none" onClick="show_tab('添加监控', '/company/monitor-add')" href="javascript:;" title="添加">添加监控</a>
			</div>
			...
			<thead>
			<tr class="text-c">
			<th width="5%"  style="height:36px">序号</th>
			...
			<c:forEach items="${list }" varStatus="index" var="be">
				<tr class="text-c">
				<td style="height:36px">${index.index+1 }</td>
				<td>${be.name }</td>
				...
				<a style="text-decoration:none" onClick="show_dialog('${be.name}', '/village/to_monitor-list?userId=${userId}&before=1&flag=3&name=${be.name }')" href="javascript:;" title="删除">查看监控</a>
				</td>
				</tr>
			</c:forEach>
...
<%--			<a href="javascript:void(0);" onclick="img_upload('pic4', null)" class="btn btn-primary radius upload-btn mt-10"><i class="Hui-iconfont">&#xe642;</i> 上传图片</a>--%>
<%--			</div>--%>
<%--			</div>--%>
<%--			</div>--%>
...
<%--		--%>
		...
		function getindex(camera,dbname){
		for(var i=0;i<dbname.length;i++){
		if(camera==dbname[i]) return i;
		}
		return -1;
		}
		...
		//+"<td data-id='"+(getindex(cameras[i],dbname)==-1?-1:dbid[getindex(cameras[i],dbname)])+"' data-img='"+(getindex(cameras[i],dbname)==-1?'null':dbimg[getindex(cameras[i],dbname)])+"'><input class='btn btn-primary size-M radius mt-10' type='button' onclick='uploadimg(this)' value='上传图片'/></td>"
		+"<td>"
