...
<%--        柱状图--%>
        ...
        // 基于准备好的dom，初始化echarts实例
        var number = ["${a}", "${b}", "${c}", "${d}", "${e}", "${f}", "${g}", "${h}", "${i}", "${j}", "${k}", "${l}", "${m}", "${n}", "${o}", "${p}", "${q}", "${r}"];
        var firstdata2 = ["${a1}", "${b1}", "${c1}", "${d1}", "${e1}", "${f1}", "${g1}", "${h1}", "${i1}", "${j1}", "${k1}", "${l1}", "${m1}", "${n1}", "${o1}", "${p1}", "${q1}", "${r1}"];
        var x="",fdata=[];
        console.log(number);
        firstdata2.map((v,i)=>{
           x=v*100;
...
<%--        fontSize:12//主题文字字体大小，默认为12px--%>
        ...
        let number11 = ["${a}", "${b}", "${c}", "${d}", "${e}", "${f}", "${g}", "${h}", "${i}", "${j}", "${k}", "${l}", "${m}", "${n}", "${o}", "${p}", "${q}", "${r}"];
        let firstdata11 = ["${a1}", "${b1}", "${c1}", "${d1}", "${e1}", "${f1}", "${g1}", "${h1}", "${i1}", "${j1}", "${k1}", "${l1}", "${m1}", "${n1}", "${o1}", "${p1}", "${q1}", "${r1}"];
        let x11="",fdata11=[];
        firstdata11.map((v,i)=>{
        x11=v*100;
        fdata11.push(x11);
        ...
        let number22= ["${a}", "${b}", "${c}", "${d}", "${e}", "${f}", "${g}", "${h}", "${i}", "${j}", "${k}", "${l}", "${m}", "${n}", "${o}", "${p}", "${q}", "${r}"];
        let firstdata22 = ["${a1}", "${b1}", "${c1}", "${d1}", "${e1}", "${f1}", "${g1}", "${h1}", "${i1}", "${j1}", "${k1}", "${l1}", "${m1}", "${n1}", "${o1}", "${p1}", "${q1}", "${r1}"];
        let x22="",fdata22=[];
        firstdata22.map((v,i)=>{
        x22=v*100;
        fdata22.push(x22);
        ...
        $("#chaBtn1").removeClass('btn-default')
        $("#chaBtn2").removeClass('btn-default')
        let number33 = ["${a}", "${b}", "${c}", "${d}", "${e}", "${f}", "${g}", "${h}", "${i}", "${j}", "${k}", "${l}", "${m}", "${n}", "${o}", "${p}", "${q}", "${r}"];
        let firstdata33 = ["${a1}", "${b1}", "${c1}", "${d1}", "${e1}", "${f1}", "${g1}", "${h1}", "${i1}", "${j1}", "${k1}", "${l1}", "${m1}", "${n1}", "${o1}", "${p1}", "${q1}", "${r1}"];
        let x33="",fdata33=[];
        firstdata33.map((v,i)=>{
