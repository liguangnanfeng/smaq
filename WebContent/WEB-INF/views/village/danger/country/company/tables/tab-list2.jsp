...
<body>
  <nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> <span>各类安全检查表</span> 
  ...
  <div class="page-container">
    <div class="text-c">
    <form action="${ly }/company/tables/tab-list2" method="post">
        <div class="dis-ib">
          <span>资料类型：</span>
          <span class="select-box inline">
          <select name="isType" class="select">
            <option value="">全部</option>
            <option value="30" <c:if test="${30==isType}">selected</c:if>>公司级安全检查表</option>
            <option value="31" <c:if test="${31==isType}">selected</c:if>>机械设备安全检查表</option>
            <option value="32" <c:if test="${32==isType}">selected</c:if>>季节性安全检查表</option>
            <option value="33" <c:if test="${33==isType}">selected</c:if>>节假日安全检查表</option>
            <option value="34" <c:if test="${34==isType}">selected</c:if>>车间级安全检查表</option>
            <option value="35" <c:if test="${35==isType}">selected</c:if>>电气设备（专业）安全检查表</option>
          </select>
          </span>
        </div>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
      <div class="l">
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=30"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看公司级安全检查表</a>
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=31"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看机械设备安全检查表</a>
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=32"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看季节性安全检查表</a>
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=33"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看节假日安全检查表</a>
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=34"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看车间级安全检查表</a>
        <a class="btn btn-primary radius mt-5 mb-5"   href="${ly}/company/tables/templetDownload?isType=35"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看电气设备（专业）安全检查表</a>
      </div>
    </div>
    <div class="mt-20">
              ...
              <div class="row cl">
                ...
                <div class="formControls col-xs-8 col-sm-9" style="width:70%;">
                  <select name="isTab" id="b" class="select" style="height:31px;">
                    <option value="">全部</option>
                    <option value="30">公司级安全检查表</option>
                    <option value="31">机械设备安全检查表</option>
                    <option value="32">季节性安全检查表</option>
                    <option value="33">节假日安全检查表</option>
                    <option value="34">车间级安全检查表</option>
                    <option value="35">电气设备（专业）安全检查表</option>
                  </select>
                </div> 
              </div>
