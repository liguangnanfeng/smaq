    <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
        ...
        if(level=='红色'){
         $('.fx_'+id).html('<font class="col-a">红色</font>');
        }else if(level=='橙色'){
         $('.fx_'+id).html('<font class="col-b">橙色</font>');
        }else if(level=='黄色'){
         $('.fx_'+id).html('<font class="col-c">黄色</font>');
        }else{
         $('.fx_'+id).html('<font class="col-d">蓝色</font>');
        }
        ...
        <a class="btn-page pagexz" href="javascript:;">评估操作</a>
        <a class="btn-page" data-href="${ly }/company/safety-system/assess5" data-title="重大风险判定"
        onclick="Hui_admin_tab(this)" href="javascript:;">重大风险评估</a>
        <a class="btn-page" data-href="${ly }/company/safety-system/assess-list" data-title="较大风险风险判定"
        onclick="Hui_admin_tab(this)" href="javascript:;">较大风险风险评估</a>
        <a class="btn-page" data-href="${ly }/company/safety-system/assess6" data-title="一般和较小风险判定"
        onclick="Hui_admin_tab(this)" href="javascript:;">一般和较小风险评估</a>
        ...
        <c:forEach items="${list }" var="be" varStatus="index">
            ...
            <td class="text-c colorChoice fx_${be.id}">
            <c:choose>
            ...
            onclick="zpLevel_save_('${be.id}',this,'${index.index}','红色')"/>
            <input class="btn btn-warning-outline size-S radius ml-5" type="button" value="橙色"
            onclick="zpLevel_save_('${be.id}',this,'${index.index}','橙色')"/>
            <input class="btn btn-warning-outline size-S radius btn-huang ml-5" type="button" value="黄色"
            onclick="zpLevel_save_('${be.id}',this,'${index.index}','黄色')"/>
            <input class="btn btn-secondary-outline size-S radius ml-5 mr-5" type="button" value="蓝色"
            onclick="zpLevel_save_('${be.id}',this,'${index.index}','蓝色')"/>
            <a class="btn-cz" style="text-decoration:none;" onClick="del_(${be.id})" href="javascript:;"
            title="删除">删除</a>
            </td>
            <%-- <c:set value="${fn:split(be.level3,'/')}" var="ls"></c:set>
             <c:set value="0" var="y"/>
