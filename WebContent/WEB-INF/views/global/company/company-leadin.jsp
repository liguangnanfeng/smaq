...
<%-- <div id="leadin" class="page-container" style="display:none">
  ...
  <nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> <span>导入企业信息</span> 
    <!-- 判断结束 -->
  ...
  <div class="page-container">
    <div class="cl pd-5 bg-1 bk-gray mt-20">
      <span class="l">
        <a class="btn btn-primary radius" data-title="导入企业"  onclick="openwin()" href="javascript:;"><i class="Hui-iconfont" style="font-size:15px;">&#xe642;</i> 导入企业</a>
        <a class="btn btn-primary radius" data-title="查看导入模板 " href="${ly}/common/download?filename=导入模板&fileurl=${ly}/upload/trouble/model.xlsx" target="_blank"><i class="Hui-iconfont" style="font-size:15px;">&#xe640;</i> 查看导入模板 </a>
        <font style="color:#f40;margin-left:20px;">通过上传EXCEL模板方式导入企业信息</font>
      </span>
    ...
    <div class="mt-20">
      <table class="table table-border table-bordered table-bg table-hover table-sort">
        ...
        <tbody>
          <!-- 循环企业 -->
          <c:forEach items="${list }" varStatus="index" var="cpy">
          <tr class="text-c">
            <td>${index.index + 1}</td>
            <td><a href="${ly }${cpy.url}" target="_blank">${cpy.name }</a></td>
            <td><fmt:formatDate value="${cpy.createTime }" pattern="yyyy-MM-dd HH:mm:ss"/></td>
          </tr>
          </c:forEach>
          <!-- 循环结束 -->
        </tbody>
...
function upload() {
  $.ajaxFileUpload({
      url: getRootPath() + '/village/company/company-import', //用于文件上传的服务器端请求地址
