...
<body>
  ...
  <div class="cl pd-5 bg-1 bk-gray mt-20">
  	<span class="l">
  	<a class="btn btn-primary radius" data-title="添加许可证" data-href="${ly }/tradeclique/clique/license-add" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加添加许可证</a>
  	</span>
          ...
          <tbody>
              ...
              <c:forEach items="${cliqueList }" varStatus="index" var="clist">
	            <tr class="text-c">
	                <td>${index.index+1 }</td>
	                <td>${cliqueName}</td>
	                <td>${type_[clist.type - 1]}</td>
	                <td>${clist.certificateNumber}</td>
	                <td><img alt="${clist.img}" src="${clist.img}" style="max-height: 100px;cursor:pointer;" onclick="showpicture('${clist.img}')"/></td>
	                <td>${clist.timeGet}</td>
	                <td>${clist.timeExp}</td>
	                <td>
		                <a style="text-decoration: none" onclick="edt(${clist.id})" href="javascript:;" title="编辑证书">编辑证书</a> 
              ...
              <c:forEach items="${list }" varStatus="index" var="list">
	            <tr class="text-c">
	                ...
	                <td>${list.timeExp}</td>
	                <td>
		                <a style="text-decoration: none" onclick="edt(${list.id})" href="javascript:;" title="编辑证书">编辑证书</a> 
...
<script type="text/javascript">
  ...
  /*编辑*/
  function edt(id) {
    show_tab("编辑各类许可证", getRootPath() + '/tradeclique/clique/license-add?id='+id);
	}

  /*删除*/
	function del(id) {
	  	layer.confirm("确认要删除此条记录吗？", function() {
		    $.post(getRootPath() + "/tradeclique/clique/license-del", {
		      	id: id
		    }, function(result) {
		        $('form').submit();
		    })
