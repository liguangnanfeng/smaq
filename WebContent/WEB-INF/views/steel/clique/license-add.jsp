    	...
    	<div class="row cl">
	      ...
	      <div class="formControls col-xs-8 col-sm-9">
	        <select id="uid" name="uid"  style="width:350px" class="input-text required">
	          ...
	          <option value="${clique.userId }"<c:if test="${standard.userId == clique.userId}">selected</c:if>>${clique.name }</option>
	          <c:forEach items="${branches }" varStatus="index" var="br">
	          	<option value="${br.userId }"<c:if test="${standard.userId == br.userId}">selected</c:if>>${br.name }</option>
	          </c:forEach>
	        </select>
	      </div> 
	    </div>
...
function save_submit() {
  if($("#uid").val() == '') {
    layer.alert("请选择所属公司",function(i) {
      layer.close(i);
      $("#uid").focus();
    })
    return false;
  ...
  if($("#e").val() == '') {
    ...
    $.post(getRootPath() + "/tradeclique/clique/license-save", {
      id : id_,
      userId: $("#uid").val(),
      type: $("#a").val(),
      img: $("#pic3").attr("url"),
      ...
      timeExp: $("#e").val()
            ...
            show_tab("各类许可证", getRootPath() + '/tradeclique/clique/license-list');
    })
}
