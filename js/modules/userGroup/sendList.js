var sendList = (function(){
  ...
  var info = {
      ...
      getList : function(page,limit,searchTitle){
        $.ajax({
          url : rootPath + "/village/getSendList",
          type : 'post',
          data : {
            page : page,
            limit : limit,
          ...
          success : function(data){
            if(undefined != data.list && data.list.length > 0){
              ...
              $("#showTotal").html(0);
              $("#showListBody").html('<tr class="text-c"><td colspan="5">暂无消息</td></tr>');
            }
            
            $(".toDel").unbind("click").bind("click",function(){
              var messageId = $(this).attr("core-id");
              layer.confirm('删除后，已接收用户也无法查看该消息，确认删除吗？', {
                btn: ['确定','取消'] //按钮
              }, function(){
                $.ajax({
                  url : rootPath + "/village/toDeleteSendMsg",
                  type : 'post',
                  data : {
                    messageId : messageId
                  },
