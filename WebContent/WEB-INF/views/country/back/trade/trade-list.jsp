...
<body>
  <nav class="breadcrumb">
    ...
    <span class="c-gray en">&gt;</span> <span>行业列表</span> 
  ...
  <div class="page-container">
    <div class="text-c">
      <form action="" method="post">
        ...
        <div class="dis-ib"><span>行业名称：</span><input type="text" value="${name }" name="name" class="input-text mb-5 mt-5" style="width:150px;"></div>
        <div class="dis-ib">
          <span>运行状态：</span>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
      <span class="l">
        <a class="btn btn-primary radius" data-title="添加行业" data-href="${ly }/back/trade-add" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont">&#xe600;</i> 添加行业</a>
      </span>
...
<!--             <th width="10%">区县地址</th>
        ...
        <tbody>
          ...
          <tr class="text-c">
            ...
            <td>
              <a style="text-decoration:none" onClick="show_tab('行业信息', '${ly}/back/trade-add?userId=${to.userId }');" href="javascript:;" title="编辑">查看/编辑</a>
        ...
        <%-- add wz 190125 start--%>
              <c:if test="${to.isClique == 0}">
              	<a style="text-decoration:none" onClick="user_clique(${to.userId})" href="javascript:;" title="设为总部企业">设为总部企业</a>
              </c:if>
              <c:if test="${to.isClique == 1}">
              	<a style="text-decoration:none" onClick="user_unclique(${to.userId})" href="javascript:;" title="取消总部企业">取消总部企业</a>
              </c:if>
        <%-- add wz 190125 end--%>     
...
//设为总部企业
function user_clique(userId){
layer.confirm("确认将该行业端账户设为总部企业吗？",function(){
  var index = layer.load();
  $.post(getRootPath() + "/back/user-clique",{
    userId : userId,
    isClique : 1
  },function(result){
    layer.close(index);
...
//取消总部企业
function user_unclique(userId){
layer.confirm("确认将该行业端账户取消总部企业吗？",function(){
  var index = layer.load();
  $.post(getRootPath() + "/back/user-clique",{
    userId : userId,
    isClique : 0
