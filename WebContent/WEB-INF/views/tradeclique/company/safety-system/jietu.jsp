...
<script type="text/javascript">
    ...
    ImgCropper.prototype = {
        ...
        initialize: function (container, handle, url, options) {
            ...
            if (this.Resize) {
                var op = this.options,
                    _resize = new Resize(this._layHandle, {Max: true, onResize: Bind(this, this.SetPos)});
                //设置缩放触发对象
        ...
        SetPreview: function () {
            if (this._view) {
                //预览显示的宽和高
                var p = this.GetPos(), s = this.GetSize(p.Width, p.Height, this.viewWidth, this.viewHeight),
                    scale = s.Height /
                        p.Height;
                //按比例设置参数
                var pHeight = this._layBase.height * scale, pWidth = this._layBase.width * scale, pTop = p.Top * scale,
                    pLeft =
                        p.Left * scale;
                //设置预览对象
...
<table width="1000" border="0" cellspacing="0" cellpadding="0">
    <tr>
        ...
        <td align="center">
            <div id="viewDiv" style="width:300px; height:300px;border:1px solid">
            </div>
        </td>
    </tr>
</table>
<a id="save" href="javascript:void(0)" onclick="save()"
   style="border: 1px solid #06c;color: #fff;background: #06c;padding: 10px 15px;margin-top: 24px;display: block;width: 200px;text-align: center; margin-left: 390px;border-radius: 3px;">确定截图</a>
...
<script type="text/javascript">
    ...
    function save() {
        ...
        // if ($("viewDiv").children[0]) {
            // console.log($("viewDiv").children[0].src);
             console.log($("dragDiv"));
                $("dragDiv").style.border=0;
            ...
            html2canvas($("dragDiv"),
                {useCORS: true, allowTaint: false, foreignObjectRendering: true, taintTest: false, scale: 1})
                .then(function (canvas) {
                    var image = canvas.toDataURL("image/png", 1);
                    console.log(image);
                    $("dragDiv").style.border='1px dashed #fff';
                    ...
                    if(image!=false){
                        jQuery.ajax({
                            url:getRootPath() + "/api/map/B001",    //请求的url地址 
                            data:{images:image},    //参数值
                            type:"POST",   //请求方式
