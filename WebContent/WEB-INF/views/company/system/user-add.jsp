...
<div class="page-container">
  <form class="form form-horizontal" id="form">
    ...
    <div class="row cl">
      <label class="form-label col-xs-4 col-sm-2"><span class="c-red">*</span><c:if test="${session_user.userType != 3}">车间/部位：</c:if><c:if test="${session_user.userType == 3}">部门/科室：</c:if></label>
      <div class="formControls col-xs-8 col-sm-9">
        <span class="select-box inline"> 
          <select name="cid" class="select" id="cid">
            <option value=""><c:if test="${session_user.userType != 3}">选择所在公司</c:if><c:if test="${session_user.userType == 3}">选择单位</c:if></option>
        ...
        <span class="select-box inline"> 
          <select name="dpid" class="select" id="dpid">
            <option value=""><c:if test="${session_user.userType != 3}">选择车间</c:if><c:if test="${session_user.userType == 3}">选择部门</c:if></option>
          </select>
        </span>
        <span class="select-box inline"> 
          <select name="did" class="select" id="did">
            <option value=""><c:if test="${session_user.userType != 3}">选择部位</c:if><c:if test="${session_user.userType == 3}">选择科室</c:if></option>
          </select>
        </span>
      </div>
    </div>
