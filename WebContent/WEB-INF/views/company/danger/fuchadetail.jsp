    <%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8" %>
        ...
        <c:choose>
            <c:when test="${check.flag == 1}">
                <button onClick="pr_()" style="background:rgb(94, 185, 94);border-color:rgb(94, 185, 94);margin-right:10px" class="btn btn-primary radius bit_hid" type="button">
                <i class="Hui-iconfont">&#xe652;</i>打印预览
<%--                </button>onClick="location.href = '/company/fuchadetail?checkId=${be.id}&flag=${flag}&number=${be.c }'"--%>
                <button  class="fudetail btn btn-success radius" style="text-decoration:none" onClick="show_dialog('整改复查意见书_${be.id }', '${ly }/company/recheck-detail?checkId=${checkId}&flag=${flag}&number=${number}')" href="javascript:;">整改复查意见书</button><br>
...
<%--                ?checkId=${be.id}&flag=${flag}&number=${be.c }--%>
                    <button class="fudetail btn btn-success radius" style="text-decoration:none" onClick="show_dialog('整改复查意见书_${be.id }', '${ly }/company/recheck-detail?checkId=${checkId}&flag=${flag}&number=${number}')" href="javascript:;">整改复查意见书</button><br>
        ...
        var checkId = searchURL.split("&")[0].split("=")[1];
        var flag = searchURL.split("&")[1].split("=")[1];
        var number = searchURL.split("&")[2].split("=")[1];
        </script>
        ...
        <c:forEach items="${recheckList }" var="p">
            ...
            <th width="6%">序号</th>     <%--1--%>
            <th width="14%">检查部位</th><%--2--%>
            <th width="14%">检查类型</th><%--3--%>
            <th width="14%">检查方式</th><%--4--%>
            <th width="24%">检查内容</th><%--5--%>
            <th width="14%">整改期限</th><%--6--%>
            <th width="14%">检查结果</th><%--7--%>
            </tr>
            </thead>
            <tbody>
            ...
            <c:forEach items="${itemList }" var="be">
                <c:if test="${be.recheckId == p.id}">
                    ...
                    <%--     6整改期限--%>
                    <td class="text-c"><fmt:formatDate value="${p.nextTime }" pattern="yyyy-MM-dd"/></td>
                    ...
                    <c:if test="${!empty be.files or !empty be.memo}">
                        ...
                        <br>
                        <c:if test="${be.status != 3}"><a href="javascript:void(0)" onclick="showpicture('${be.files}',
                            this)">隐患描述</a>
                        </c:if>
            ...
            <c:if test="${check.flag == 3 && check.checkCompany eq '无锡市安泰安全技术服务有限公司'}">
                <img alt="" src="${ly }/images/zhang.png" style="position:
                absolute;bottom:10px;right:10px;height:130px;"/>
            </c:if>
            ...
            <div class="div_pright mt-10 mb-10">
            <input type="text" style="width:150px" value="${p.checker }" class="input-text" maxlength="50"
            disabled="disabled"/>
            </div>
            ...
            <div class="div_pright mt-10 mb-10" >
            <input type="text" style="width:150px" value="${check.dapartContact }" class="input-text" maxlength="50"
            disabled="disabled"/>
            </div>
