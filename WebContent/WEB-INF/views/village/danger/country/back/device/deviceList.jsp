...
<body>
  ...
  <div class="text-c">
      ...
      <div class="dis-ib">
          <span>请选择单位：</span>
          <select id="companySelect" class="input-text mb-5 mt-5" style="width:150px;">
          	<option value="">全部</option>
          	<c:forEach items="${list }" var="cp">
          		<option value="${cp.userId }">${cp.companyName }</option>
          	</c:forEach>
          </select>
        </div>
        <div class="dis-ib">
          <span>设备名称：</span>
          <input type="text" id="searchDeviceInput" value="" class="input-text mb-5 mt-5" style="width:150px;">
        </div>
        <button class="btn btn-success" type="button" id="toSearchDeviceName">
...
{@each list as m,index}
    <tr class="text-c">
          ...
          <td>$<{m.sensorsCount}></td>
		  <td>{@if m.status == 1}
				<span style="color:#62c462;">在线</span>
			  {@else}
				<span style="color:#666;">离线</span>
			  {@/if}
		  </td>
          ...
          <td><a href="<c:url value='/back/toAddEquipment?deviceId=$<{m.id}>'/>">编辑</a>
			 <a href="javascript:void(0)" core-snid="$<{m.deviceSn}>" class="toDel">删除</a>
			</td>
     </tr>
{@/each}
</script> 
