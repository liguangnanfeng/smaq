...
<body>
  ...
  <div class="page-container">
    <div class="text-c">
      <form action="${ly }/tradeclique/clique/evaluation-list" method="post">
        <div class="dis-ib">
          <span>评价报告名称：</span>
    ...
    <div class="cl pd-5 bg-1 bk-gray mt-20">
    	<span class="l">
        <a class="btn btn-primary radius" data-title="添加评价评估" data-href="${ly }/tradeclique/clique/evaluation-add" onclick="Hui_admin_tab(this)" href="javascript:;"><i class="Hui-iconfont" style="font-size:15px;">&#xe600;</i> 添加评价评估</a>
      	</span>
      ...
      <table class="table table-border table-bordered table-bg table-hover table-sort">
        ...
        <tbody>
          ...
          <tr class="text-c">
            ...
            <td>${cliqueName }</td>
            <td>
            	<c:if test="${cl.flag==1 && cl.type==1}">安全生产类--预评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==2}">安全生产类--验收评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==3}">安全生产类--现状评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==4}">安全生产类--危化品经营证评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==5}">安全生产类--危化品生产证评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==6}">安全生产类--危化品使用证评价</c:if>
            	<c:if test="${cl.flag==1 && cl.type==7}">安全生产类--重大危险源评估</c:if>
            	<c:if test="${cl.flag==1 && cl.type==8}">安全生产类--安全条件论证报告</c:if>
            	<c:if test="${cl.flag==1 && cl.type==9}">安全生产类--安全条件综合分析报告</c:if>
            	<c:if test="${cl.flag==2 && cl.type==1}">职业健康类--预评价</c:if>
            	<c:if test="${cl.flag==2 && cl.type==2}">职业健康类--控效评价</c:if>
            	<c:if test="${cl.flag==2 && cl.type==3}">职业健康类--现状评价</c:if>
           	</td>
            <td>${cl.reportName }</td>
            <td>${cl.reportTime }</td>
            <td>${cl.expirationTime }</td>
            <td>${cl.unit }</td>
            <td>${cl.docName }</td>
            <td>
            <c:if test="${!empty cl.docUrl}">
              <a style="text-decoration:none" href="${ly}/common/download?filename=${cl.docName}&fileurl=${cl.docUrl}" title="查看文档" target="_blank">查看文档</a>
            </c:if>
            <a style="text-decoration:none" onClick="admin_edit(${cl.id})" href="javascript:;" title="编辑">查看/编辑</a>
...
function admin_edit(id){
  show_tab("编辑评价评估", getRootPath() + "/tradeclique/clique/evaluation-add?flag=${flag}&id=" + id);
}

/*删除*/
function del(id){
  layer.confirm("确认要删除吗？",function(r){
    $.post(getRootPath() + "/tradeclique/clique/evaluation-del", {
      id : id,
    },function(result) {
      $("form").submit();
    });
