...
var P_LY = 
{
    ...
    */
    ...
    {
	    type:'',
	  	ver:'2.0.16.826',	// webcu
	  	eVer:'2.0.16.815',// embed
...
/*    			if (typeof document.getElementById("P_LYWAP").DebugSwitch == "undefined")
    ...
    **/
    ...
    desc: 获取逻辑分组
    returns:
    	- succ <response: Array(P_LY.Struct.PUGroupStruct)>
    params:
		- connectId(string) * 连接ID
    ... 
    */
	GetPUGroups : function (connectId)
	{
		try
		{
			var fn = "P_LY.GetPUGroups";
            ...
            else 
            {
				...
				var requestXML = '<?xml version="1.0" encoding="utf-8"?><M Type="CusReq"><C Type="C" OptID="C_CAS_QueryPUGroupInfo"/></M>';			
				...
				if (operator.rv == P_Error.SUCCESS)
				{
					...
					if (typeof XML != "undefined" && typeof XML.ObjTree != "undefined")
					{
						...
						if (typeof _response == "object" && _response.M && _response.M.C && typeof _response.M.C.SPError != "undefined" && _response.M.C.SPError != "0")
						{
							// - 返回具体的NUErrorCode错误码
							operator.rv = _response.M.C.SPError || -1;
						}
						else if (typeof _response == "object" && _response.M && _response.M.C && _response.M.C.PUGroup) 
						{
							PUGroup = _response.M.C.PUGroup || [];
							if (typeof PUGroup == "object")
							{
								if (PUGroup.constructor != Array)
								{
									PUGroup = [PUGroup];	
								}
								for (var i = 0; i < PUGroup.length; i++)
								{
									_lgsArray.push
									(
									 	new P_LY.Struct.PUGroupStruct
										(
										 	PUGroup[i].Index, 
											PUGroup[i].Name, 
											PUGroup[i].RefreshTime, 
											PUGroup[i].RefreshInterval
										)
									);	
								}
							}
    ...
    desc: 获取逻辑分组子节点
	returns:
		- succ <response: Array(P_LY.Sruct.PUGroupNodeStruct)>
	params:
		- connectId(string) * 连接ID
		- options(object) * 可选参数
		{
			puGroupIndex(uint<P_LY.Struct.PUGroupStruct:index>) * 逻辑分组索引
			puGroupNodeIndex(uint) 逻辑分组节点索引，缺省为0
	...
	remark:
		- 获取逻辑分组节点或节点下节点
    	- puGroupNodeIndex表示当前节点（父节点），根节点的父节点索引为0，即缺省获取逻辑分组直接子节点
		- fetchMode = 'step'时，如果puGroupNodeIndex属性存在，就获取这个节点下面的节点，不递归。如果不存在此属性，则获取所有节点
		- fetchMode = 'once'时，无论puGroupNodeIndex存不存在，则获取puGroupIndex下面的所有节点
		- count 默认为一个足够大的数，但是如果节点过大的话，建议配合offset进行分页获取
	...
	*/
	GetPUGroupNodes : function (connectId, options)
	{
		try
		{
			var fn = "P_LY.GetPUGroupNodes";
			if (!connectId || !P_LY.Connections.get(connectId)) {
                P_Utils.Log(fn, "connectId error~");
                return new P_LY.Struct.ReturnValueStruct(P_Error.ERROR_CONNECTID_ERROR);
            }
            ...
            {
				var o = options || {};
				if (typeof o.puGroupIndex == "undefined" || !P_LY.intRex.test(o.puGroupIndex))
				{
					P_Utils.Log(fn, "o.puGroupIndex error~");
					return new P_LY.Struct.ReturnValueStruct(P_Error.ERROR_INDEX_ERROR); 
				}
				
				var _connStruct = P_LY.Connections.get(connectId);
				
				var xmlParam = '<PUGroup Index="'+(Number(o.puGroupIndex)||0)+'" ';
				
				o.puGroupNodeIndex = typeof o.puGroupNodeIndex != "undefined" && !isNaN(o.puGroupNodeIndex) ? Number(o.puGroupNodeIndex) : 0;
				...
				{
					xmlParam += ' NodeIndex="'+o.puGroupNodeIndex+'" ';
				}
				
				xmlParam += ' Offset="'+o.offset+'" Count="'+o.count+'" />';
				
				var requestXML = '<?xml version="1.0" encoding="utf-8"?><M Type="CusReq"><C Type="C" OptID="C_CAS_QueryPUGroupNode">'+xmlParam+'</C></M>';
				...
				if (operator.rv == P_Error.SUCCESS)
				{
					...
					if (typeof XML != "undefined" && typeof XML.ObjTree != "undefined")
					{
						...
						else if (typeof _response == "object" && _response.M && _response.M.C && _response.M.C.Node) 
						{
							Node = _response.M.C.Node || [];
							if (typeof Node == "object")
							{
								...
								for (var i = 0; i < Node.length; i++)
								{
									_lgnsArray.push
									(
									 	new P_LY.Struct.PUGroupNodeStruct
										(
										 	Node[i].Index, 
											Node[i].Name, 
	...
	/*
	---
	fn: GetPUGroupResource
    desc: 获取逻辑分组节点下资源
	returns:
		- succ <response: Array(P_LY.Sruct.PUGroupResourceStruct)>
	params:
		- connectId(string) * 连接ID
		- options(object) * 可选参数
		{
	...
	remark:
    	...
    	- puGroupNodeIndex表示当前节点（父节点），根节点的父节点索引为0，即缺省获取逻辑分组直接子节点资源
		- fetchMode = 'step'时，如果puGroupNodeIndex属性存在，就获取这个节点下面的节点资源，不递归。如果不存在此属性，则获取所有节点资源
		- fetchMode = 'once'时，无论puGroupNodeIndex存不存在，则获取puGroupIndex下面的所有节点资源
		- count 默认为一个足够大的数，但是如果节点过大的话，建议配合offset进行分页获取
	...
	*/
	GetPUGroupResource : function (connectId, options)
	{
		try
		{
			var fn = "P_LY.GetPUGroupResource";
			if (!connectId || !P_LY.Connections.get(connectId)) {
                P_Utils.Log(fn, "connectId error~");
                return new P_LY.Struct.ReturnValueStruct(P_Error.ERROR_CONNECTID_ERROR);
            }
            else 
            {
				...
				var requestXML = '<?xml version="1.0" encoding="utf-8"?><M Type="CusReq"><C Type="C" OptID="C_CAS_QueryPUGroupResource">'+xmlParam+'</C></M>';
				...
				if (operator.rv == P_Error.SUCCESS)
				{
					...
					if (typeof XML != "undefined" && typeof XML.ObjTree != "undefined")
					{
						...
						else if (typeof _response == "object" && _response.M && _response.M.C && _response.M.C.Res) 
						{
							Res = _response.M.C.Res || [];
							if (typeof Res == "object")
							{
								...
								for (var i = 0; i < Res.length; i++)
								{
									_lgrsArray.push
									(
									 	new P_LY.Struct.PUGroupResourceStruct
										(
										 	Res[i].PUID, 
											Res[i].Type, 
											Res[i].Idx, 
			...
			try
			{
				...
				else 
				{
					...
					[{
						...
						_method: "GetPUModelName", _property: "modelName", _value: "" 		 
					},{
						_method: "GetPUManufactureID", _property: "manufactrueID", _value: "" 		 
					},{
						/*
...
//				}
				...
				var iaStreamHandle = operator.response;
				if (iaStreamHandle) {
					P_IF.CloseAudio(_connStruct.nc, iaStreamHandle);
				}
				// - 设置是否重绘背景标志
			...
			if (objWnd && objWnd.wnd) 
			{
				...
				if (connectId && P_LY.Connections.get(connectId)) 
				{
					...
					if (objWnd.status.playvideoing) 
					{
						...
						{	
							P_IF.CloseAudio(_conn.nc, ivStreamHandle);
						}
		...
		try
		{
			...
			if (objWnd && objWnd.wnd) 
			{
				...
				if (objWnd.status && objWnd.status.playvideoing)
				{
					if (objWnd.status.playaudioing)
					{
						var iaStreamHandle = objWnd.params.ivStreamHandle;
						if (iaStreamHandle) {
							P_IF.CloseAudio(_connStruct.nc, iaStreamHandle);
						}
					//	objWnd.params.iaStreamHandle = "";
					...
					else
					{
						...
						var operator = P_IF.OpenAudio(_connStruct.nc, objWnd.params.ivStreamHandle);
...
　　		- 点播前端CEFS录像，type=P_LY.Enum.StorageFileType.CEFS时，可选下列参数传入puid、ivIndex、speed、direction、beginTime、endTime
    ...
    **/
	UserManagementControl : 
	{
		...
		// - 获取一个用户的逻辑分组
		QueryUserPUGroup : function (connectId, userIndex, nuType_cui, nuid_cui)
		{
			return this.__Sdk_UMCtl
			(
			 	"QueryUserPUGroup", 
		...
		// - 添加用户逻辑分组
		AddUserPUGroup : function (connectId, userIndex, puGroupIndexs, nuType_cui, nuid_cui)
		{
			return this.__Sdk_UMCtl
			(
				"AddUserPUGroup",
		...
		// - 移除用户逻辑分组
		RemoveUserPUGroup : function (connectId, userIndex, puGroupIndexs, nuType_cui, nuid_cui)
		{
			return this.__Sdk_UMCtl
			(
				"RemoveUserPUGroup",
			...
			try 
			{
				...
				switch (action)
				{
					...
					case "QueryUserResource":
						...
						break;
					case "QueryUserPUGroup":
						xmlRequest = '<?xml version="1.0" encoding="utf-8" ?><M Type="ComReq"><C Type="C" OptID="C_CAS_QueryUserPUGroup"><User Index="'+options.userIndex+'" /></C></M>';	
						break;
					case "QuerySubUsers":
					...
					case "RemoveUserResource":
						...
						break;
					case "AddUserPUGroup":
					case "RemoveUserPUGroup":
						if (!options.puGroupIndexs || typeof options.puGroupIndexs != "object")
						{
							P_Utils.Log(fn, "options.puGroupIndexs null ...");
							return new P_LY.Struct.ReturnValueStruct(P_Error.ERROR);	
						}
						if (options.puGroupIndexs.constructor != Array)
						{
							options.puGroupIndexs = [options.puGroupIndexs];	
						}
						var xmlRes = '';
						for (var i = 0; i < options.puGroupIndexs.length; i++)
						{
							xmlRes += '<PUGroup Index="'+options.puGroupIndexs[i].Index+'" />';	
						}
						var operationID = (action == 'AddUserPUGroup' ? 'C_CAS_AddUserPUGroup' : 'C_CAS_RemoveUserPUGroup');
						xmlRequest = '<?xml version="1.0" encoding="utf-8" ?><M Type="ComReq"><C Type="C" OptID="'+operationID+'"><User Index="'+options.userIndex+'">'+xmlRes+'</User></C></M>';
						break;
						...
						{ 
							switch (action)
							{
								...
								case "QueryUserResource":
									...
									break;
								case "QueryUserPUGroup":
									var PUGroup = jsonResponse.Msg.Cmd.PUGroup;
									if (typeof PUGroup == "object")
									{
										if (PUGroup.constructor != Array) PUGroup = [PUGroup];
										operator.response = PUGroup;
									}
									else
									{
										operator.rv = P_Error.FAILED;	
	...
	Control : 
	{
		...
		*/
		CommonGet : function(connectId, reqParams)
		{
			...
			}else if(reqParams.controlID == "C_GS_QueryHistoryGPSData" || reqParams.controlID == "C_GS_QueryLastGPSData"){
    ...
    { 
        ...
        */
        ...
        params:
			...
			- childResource(P_LY.Struct.PUGroupNodeStruct) 逻辑分组节点数组对象 
        */
		PUGroupStruct : function (index, name, lastRefreshTime, refreshInterval, childResource)
		{
        ...
        params:
			...
			- childResource(P_LY.Struct.PUGroupNodeStruct|P_LY.Struct.PUGroupResourceStruct) 数组对象 
        */
		PUGroupNodeStruct : function (index, name, parentNode_Index, childResource)
		{
			this.index = (index != null && typeof index != "undefined" ? index : "0");
            this.name = (name != null && typeof name != "undefined" ? name : "");
        ...
        */
		PUGroupResourceStruct: function (puid, type, idx, name, description, enable, parentNode_Index)
		{
