...
<div class="page-container">
  <form class="form form-horizontal" id="form">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <c:choose>
          <c:when test="${be.flag == 1}">
            <input type="text" name="name" disabled="disabled" readonly="readonly" value="${empty be ? session_user.userName : be.name }" style="width:550px" class="input-text" placeholder="如：XXXX公司"/>
          </c:when>
          <c:when test="${be.flag == 2}">
            <input type="text" name="name" disabled readonly value="${be.name }" style="width:550px" class="input-text" placeholder="如：XXXX车间"/>
          </c:when>
          <c:otherwise>
            <input type="text" name="name" disabled readonly value="${be.name }" style="width:550px" class="input-text" placeholder="如：XXXX班组/岗位"/>
          </c:otherwise>
         </c:choose>
    ...
    <c:if test="${be.level == '红色'}"><c:set value="红" var="fxdj1"/></c:if>
    <c:if test="${be.level == '橙色'}"><c:set value="橙" var="fxdj2"/></c:if>
    <c:if test="${be.level == '黄色'}"><c:set value="黄" var="fxdj3"/></c:if>
    <c:if test="${be.level == '蓝色'}"><c:set value="蓝" var="fxdj4"/></c:if>
    </c:forEach>
    ...
    <div class="row cl">
      ...
      <div class="formControls col-xs-8 col-sm-9">
        <input type="text" name="glzrr" value="${be.glzrr }" style="width:550px" class="input-text" placeholder="请输入管理责任人"/>
      </div> 
    </div>
    <div class="row cl">
