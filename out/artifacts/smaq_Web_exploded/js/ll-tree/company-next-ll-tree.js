$(function(){
  ...
  for(var i=0;i<levelL.length;i++) {
    ...
    if(!list1.contains(l)) {
      list1.push(l);
      $(".list_danger").append(
        $('<div class="one_danger one_dangerxz" data-t="'+l+'"><div class="one_dtitle"><i class="Hui-iconfont">&#xe631;</i><p>'+l+'</p></div></div>')
      )
    }
  ...
  $.each(itemL, function(i, item) {
    ...
    if(p.hasClass("three_danger") != '') {//用四级样式
      ...
      p.find(".list_thdan").append(
        $('<div class="four_danger" data-end="1" data-itemid="'+item.id+'"><div class="four_dtitle"><p class="p_four"><bb>&nbsp;</bb>'
            + '<img alt="" class="danger_" src="/images/fore/zd.jpg" '+(item.status > 1 ? 'style="display:inline"':'') +'/>'
            + c +
            '</br><font class="font_danger">'
            +c2+
            '</font></p><button class="btn btn-danger ml-10 radius" type="button" onclick="openzl(this)">不合格</button>' + 
            '</div></div>')    
      )
    } else {
      ...
      p.find(".list_twdan").append(
        $('<div class="three_danger" data-end="1" data-itemid="'+item.id+'"><div class="three_dtitle"><p class="p_three"><bb>&nbsp;</bb>'
            + '<img alt="" class="danger_" src="/images/fore/zd.jpg" '+(item.status > 1 ? 'style="display:inline"':'') +'/>'
            + c +
            '</br><font class="font_danger">'
            +c2+
